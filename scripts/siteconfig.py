...
# How old skill-authorities.json may get before heartbeat.yml files a tracking issue, in days.
#
# That manifest is the join key for alert-out: skill_alert.py reads its by_authority index to decide
# which adverse treatments matter. It is generated by scripts/skill_authorities.py from the qpwb
# skill tree at /mnt/skills/user -- a path NO workflow has, so it cannot regenerate in CI. It is
# regenerated by hand and committed, and between those commits it is frozen against a skill tree
# that keeps moving. Nothing about that state errors: skill_alert fails open on a missing or
# unreadable manifest, and a merely OUT-OF-DATE one is worse than a missing one, because it looks
# like a working watch while quietly watching last quarter's authorities.
#
# So the age is checked instead. Set to 0 to disable the check (e.g. if you stop maintaining the
