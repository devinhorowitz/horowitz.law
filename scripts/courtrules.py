...
EXTRACT_SYSTEM = (
    ...
    "FRBP), rule (e.g. 'Rule 26' or 'Rule 702'), a one-sentence plain-English summary, status "
    "('pending' or 'effective'), effective_date (YYYY-MM-DD if the page states one, else empty), "
    "and impact.\n\n"
    "THIS PAGE IS USUALLY A DOCKET, NOT A DESCRIPTION. It typically lists a rule number and a date "
    "and says nothing about what changes. That is normal and it is still worth reporting -- a "
    "litigator wants to know Rule 26 moves on a given date. But say only what the page says. When "
    "the page does not describe the change, the summary states that the rule is among the "
    "amendments projected to take effect on that date and stops there.\n\n"
    "IMPACT MUST COME FROM THE PAGE, AND IS USUALLY EMPTY. Return \"\" for impact unless the page "
    "itself says why the change matters. Do NOT reason from what you know the rule covers: writing "
    "that a Rule 902 amendment 'may alter trial exhibit preparation because self-authentication is "
    "often used for electronic records' is your inference about the rule, not the page's statement "
    "about the amendment, and on a site lawyers rely on that is a fabricated consequence attached "
    "to a real citation. An empty impact is correct and expected here.\n\n"
    "Ground every field in the page text; do not invent a rule, date, or change that is not there. "
    "If the page names no such amendment, return an empty list. Reply with ONLY a JSON object: "
    "{\"amendments\": [{\"rule_set\":..., \"rule\":..., \"summary\":..., \"status\":..., "
    "\"effective_date\":..., \"impact\":...}]}."
)
