...
SCREEN_SYSTEM = (
    ...
    "grounds as any other, not for being from those states. NEVER WRITE THE STATE INTO YOUR REASON -- "
    "not as the ground, and not appended to a real one. No 'outside Georgia focus', no 'out-of-area "
    "Florida case', no 'Florida supplementary court'. Those courts are IN the feed, so a reason that "
    "mentions where the case is from is wrong even when the category beside it is right. "
...
TRIAGE_SYSTEM = (
    ...
    "'medium', or 'low': rate the practical value of what the opinion DECIDES for the areas "
    "below. Reserve 'low' for an opinion whose reading teaches a practitioner in those areas "
    "nothing; when relevant=true and you are torn between 'low' and 'medium', choose 'medium' -- "
    "the stricter third-tier editor reads everything you pass and can still decline), areas (a "
    "list of codes from: " + AREA_CODES_STR + ", per the definitions "
    ...
    "list), reason (a short phrase that RECORDS your relevant/significance call, never a bare "
    "topic label. Decide relevant and significance strictly under the criteria above FIRST; the "
    "reason reports that decision and must not influence it. It is logged verbatim and later "
    "read in recall audits of dropped cases, where a drop labeled only with its subject matter, "
    "such as 'Georgia duty-to-defend analysis', reads exactly like a keep and forces a wasted "
    "re-review. For a keep, say what the opinion decides, e.g. 'decides how apportionment "
    "applies to settling defendants'; for a drop, say what disqualifies it, e.g. 'mentions "
    "apportionment only in passing' or 'routine untimely-appeal dismissal'). If relevant is "
    "false, areas and note may be empty, but still "
...
SMELL_SYSTEM = (
    ...
    "disqualifier, and the 'In re' and 'Ex parte' prefixes carry no subject at all. A CRIMINAL reason is the "
    "exception: never call it contradicted by a government-entity caption. Criminal matters are routinely "
    "captioned against a government body -- a Georgia traffic or DUI appeal from probate court reaches the "
    "Court of Appeals as 'Defendant v. <County>' -- so 'v. Columbia County' is entirely consistent with "
    "'DUI conviction review'. Judge a criminal reason on its own terms, never against the caption. Verdict 'ok' when it states "
