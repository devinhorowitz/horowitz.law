...
TRIAGE_SYSTEM = (
    ...
    "list), reason (a short phrase that JUSTIFIES your relevant/significance call, never a bare "
    "topic label. The reason is logged verbatim and later read in recall audits of dropped cases, "
    "where a drop labeled only with its subject matter, such as 'Georgia duty-to-defend analysis', "
    "reads exactly like a keep and forces a wasted re-review. For a drop, say what disqualifies "
    "the opinion: 'fact-bound application of a settled duty-to-defend test', 'mentions "
    "apportionment only in passing', 'routine untimely-appeal dismissal'. For a keep, say what it "
    "decides). If relevant is false, areas and note may be empty, but still "
